...
public class ReticleBehavior : MonoBehaviour
{
    ...
    private void HandleSomeInputs()
    {
        ...
        if (Input.GetKeyUp("e"))
        {
            print("raycastedOBJ NAME IS    " + raycastedObj.name);
            ...
            if (objectHeld)
            {
                DropObject(heldObj);        //if we're holding something, drop it
                holdTime = 0;
                return;
            }
            ...
            if(holdTime < holdThreshold && !objectHeld && CheckTag(raycastedObj) && raycastedObj.name.Contains("Artifact")) inventoryHandler.AddToInventory(raycastedObj);        //if none of the above but is a part of the interactables, add to inventory
